...
try:
    from .interfaces import LLMProvider, TranscriberProvider, TTSProvider
except ImportError:
    from interfaces import LLMProvider, TranscriberProvider, TTSProvider
...
class VoiceComponentFactory:
    ...
    def _create_polly_synthesizer(self, config: Dict[str, Any]):
        ...
        return PollySynthesizer(
            ...
            engine=config.get("pollyEngine", "standard"),
            output_format=config.get("pollyOutputFormat", "mp3"),
        )
