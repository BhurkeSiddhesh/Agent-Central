...
def suggest(
    ...
    *,
    dry_run: bool = typer.Option(default=False, "--dry-run", help="Only print results"),
):
    """Suggests a skill set for a project without copying files."""
    project_path = Path(project).resolve()
    config_path = project_path / "agency.yaml"
    if not config_path.exists():
        typer.echo(f"❌ No 'agency.yaml' found in project: {project_path}")
        return

    try:
        raw = yaml.safe_load(config_path.read_text(encoding="utf-8")) or {}
    except yaml.YAMLError as e:
        typer.echo(f"❌ Failed to parse agency config: {e}")
        return

    try:
        config = AgencyConfig.model_validate(raw)
    except ValidationError as e:
        typer.echo(f"❌ Invalid agency config: {e}")
        return
    ...
    typer.echo(f"📎 Suggested {len(selection['selected'])} skills:")
    ...
    if not dry_run:
        ...
        typer.echo("✔ Profile, manifest, and lockfile written to .ai-context")
